...
from src.dataset.denoise import DenoisingDataset, DenoiseTestPhase
...
def train_engine(layers, config):
    ...
    dataset = {
        "train": DenoisingDataset(
            phase=config["trainset"],
            stride=dataset_stride,
            normalize=config["normalize"],
            clip=config["clip"],
            sigma=config["sigma_range"][0],
            output_mode=config['output_mode']
        ),
        "val": DenoisingDataset(
            phase=config["valset"],
            stride=dataset_stride,
            normalize=config["normalize"],
            clip=config["clip"],
    ...
    for epoch in range(initial_epoch, n_epoch):
        logger.info(f"Epoch: {epoch + 1}".center(50, "="))
        ...
        for phase in ["train", "val", "test"]:
            ...
            if phase == "train":
                ...
                logger.info(
                    f"epoch = {epoch + 1} , loss = {round(epoch_loss[phase] / i, 4)}"
                )
...
if __name__ == "__main__":
    with open(f"../config/train_config.yml") as f:

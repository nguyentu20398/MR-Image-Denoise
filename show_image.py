...
def display_images():
    ...
    stacked_image = np.zeros((2 * height + 60, 8 * width + 70, 3), dtype=np.uint8)

    # Create a text matrix to write text
    text_matrix = np.zeros((30, 8 * width + 70, 3), dtype=np.uint8)
    ...
    # Define image names
    image_names = ['Original', 'Noise', 'BM3D', 'Strategy1', 'Strategy2', 'Strategy3', 'Strategy4', 'Strategy5']
...
if __name__ == "__main__":
    ...
    while True:
        ...
        bm3d_path = f"report/LV_ThS_TuPhuongNguyen/LuanVan/bm3d/sigma{sigma}/img_0{numer_path}_sigma_{sigma}_bm3d.png" if numer_path < 10 else f"report/LV_ThS_TuPhuongNguyen/LuanVan/bm3d/sigma{sigma}/img_{numer_path}_sigma_{sigma}_bm3d.png"
        bm3d = cv2.imread(bm3d_path)
        images.append(bm3d)
        show_images.append(bm3d)
        ...
        if waitkey == ord('s'):
            ...
            for i, name in enumerate(['Original', 'Noise', 'BM3D', 'Strategy1', 'Strategy2', 'Strategy3', 'Strategy4', 'Strategy5']):
